...
# ===> Enable command tracing, every executed command is printed to terminal 
set -o xtrace

# Make sure custom grep options don't get in the way
# ===> Remove custom grep option that may break script 
unset GREP_OPTIONS
...
# ===> Force UTF-8 English locale
LC_ALL=en_US.utf8
export LC_ALL

# Clear all OpenStack related envvars 
# ==> Remove old openstack environment variables
...
# ===> Store absolute path of devstack directory
...
# ===> Remove previous environment cache file 
...
# ===> Stop installation if files are missing
FILES=$TOP_DIR/files
if [ ! -d $FILES ]; then
    set +o xtrace
...
# ===> Prevent runninng Devstack as root
if [[ $EUID -eq 0 ]]; then
    set +o xtrace
...
#
# ===> Prevent running inside Python virtual environment
...
#
# ===> Prevent accidental Devstack installation on important systems
...
#
# ===> Initialize spinner process variable
LAST_SPINNER_PID=""
...
# Phase: local
# ===> Extract localrc section from local.conf
...
# ===> Write Devstack version information into /etc/devstack-version
...
# ===> Check whether current distro is supported
...
# ===> Export proxy variables for child processes
...
#
# ===> Ensure /etc/sudoers.d drop-in directory is included
...
#
# ===> Configure passwordless sudo for stack user
TEMPFILE=`mktemp`
...
# ===> Display animated spinner character in terminal
function spinner {
    local delay=0.75
    local spinstr='/-\|'
    printf "..." >&3
...
# ===> Print to summary log; show spinner if not in verbose mode
...
# ===> Print message to stdout only, skip log files
function echo_nolog {
    echo $@ >&3
}
...
# ===> Configure timestamped log file names and rotation
...
# ===> Trap EXIT: clean up jobs, spinner, write subunit result, dump world state
trap exit_trap EXIT
function exit_trap {
    local r=$?
    jobs=$(jobs -p)
...
# ===> Trap ERR: print failure message and exit with error code
trap err_trap ERR
function err_trap {
    local r=$?
    set +o xtrace
...
# ===> Clear any existing CA certificate bundle
SSL_BUNDLE_FILE="$DATA_DIR/ca-bundle.pem"
rm -f $SSL_BUNDLE_FILE

# Import common services (database, message queue) configuration
# ===> Load database and RPC backend library functions
...
# ===> Run plugin override_defaults phase before any service setup
run_phase override_defaults

# Import Apache functions
# ===> Load Apache, TLS, and all OpenStack service library functions
...
# ===> Prompt user for password if not set; save to localrc for reuse
...
if initialize_database_backends; then
    ...
    # ===> Prompt for and set database password, then define connection URL
...
if [[ "$OFFLINE" != "True" ]]; then
    # ===> Upgrade pip to latest upstream version
    PYPI_ALTERNATIVE_URL=${PYPI_ALTERNATIVE_URL:-""} $TOP_DIR/tools/install_pip.sh
fi

# Do the ugly hacks for broken packages and distros
# ===> Apply distro-specific package workarounds
source $TOP_DIR/tools/fixup_stuff.sh
fixup_all

if [[ "$GLOBAL_VENV" == "True" ]] ; then
    # TODO(frickler): find a better solution for this
    # ===> Symlink venv binaries into /usr/local/bin for global access
...
# ===> Disable journald rate limiting and restart journal service
...
# ===> Initialize and configure the database backend
if is_service_enabled $DATABASE_BACKENDS; then
    configure_database
fi
...
# ===> Start dstat system monitoring tool
start_dstat
...
# ===> Generate clouds.yaml auth config and select admin cloud context
write_clouds_yaml
...
# hosts.
# ===> Set shared encryption key for Cinder and Nova key manager
...
# ===> Persist generated environment variables to .stackenv cache
save_stackenv
...
# Apply configuration from ``local.conf`` if it exists for layer 2 services
# Phase: extra
# ===> Apply [extra] config overrides from local.conf
...
# Phase: extra
# ===> Execute plugin extra phase hooks
...
# Phase: post-extra
# ===> Apply [post-extra] config overrides from local.conf
...
# ===> Verify all enabled services are up and healthy
service_check
...
# ===> Install bash tab-completion for the openstack CLI
...
# Phase: test-config
# ===> Execute plugin test-config phase hooks
run_phase stack test-config

# Apply late configuration from ``local.conf`` if it exists for layer 2 services
# Phase: test-config
# ===> Apply [test-config] config overrides from local.conf
...
# ===> Kill any lingering background async processes
async_cleanup

# Dump out the time totals
# ===> Print per-service timing summary
time_totals
async_print_timing
...
# ===> Close all redirected file descriptors opened for logging
exec 1>&3
exec 2>&3
exec 3>&-
exec 6>&-
